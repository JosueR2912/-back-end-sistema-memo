...
export class MemoController {
    ...
    async create(@Body() body: Memos): Promise<any> {
        ...
        let command: string = `INSERT INTO memos(codigo_memo, de, copia_para, asunto, contenido, fecha, fromDepartamento, toDepartamento, redactadoPor ,status, status_delete, id_user) VALUES ('${body.codigo_memo}', '${body.de}', '${body.copia_para || 0}', '${body.asunto}', '${body.contenido}', STR_TO_DATE('${body.fecha}', '%Y-%m-%dT%H:%i:%s.%fZ'), ${body.fromDepartamento}, ${body.toDepartamento},'${body.redactadoPor || ''}','${body.status}', '${body.status_delete}', ${body.id_user || 0})`;
    ...
    async findAll(@Query('pageNumber') pageNumber, @Query('pageSize') pageSize): Promise<any> {
        ...
        const items: any = {};
        await SqlHelper.run(connection, `SELECT * FROM memos ORDER BY fecha DESC LIMIT ${pageNumber* pageSize}, ${pageSize}`, items);
